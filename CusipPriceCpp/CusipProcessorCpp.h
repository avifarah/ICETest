...
public:
	CusipProcessorCpp(ifstream& feedStream);
	~CusipProcessorCpp();
...
/*
	...
	{
		private readonly StreamReader feedStream;
		private int currentLineCount = 0;

		public CusipProcessor(StreamReader feedStream) => feedStream = feedStream;
		...
		public IEnumerable<CusipLatestPrice> ReadPricesForCusips(string cusip)
		{
			...
			for (; ; )
			{
				if (feedStream.EndOfStream) yield break;
				...
				for (; ; )
				{
					if (feedStream.EndOfStream)
					{
						if (prevPrice == decimal.MinValue) throw new FeedException($"Cusip {cusip} contains no price value", currentLineCount);
					...
					var line = Task.Run(feedStream.ReadLineAsync).Result;
					...
					{
						throw new FeedException("A blank line in the middle of the file indicates a corrupt Feed file", currentLineCount);
					}
					...
					if (!price.HasValue)
					{
						...
						if (nextPrice == null) throw new FeedException($"Line is neither a cusip nor a price", currentLineCount, line);
						if (prevPrice == decimal.MinValue) throw new FeedException($"Cusip {cusip} contains no price value", currentLineCount, line);
